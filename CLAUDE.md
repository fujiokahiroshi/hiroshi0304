...
- `export_cad_model_to_unity`: Rerun a model source (example or job) and write its geometry, hierarchy, colors, and full OCP Viewer animation as one Unity-importable `.glb` to `runtime/unity_exports/`. Defaults to the latest job.
- `get_viewer_selection`: ユーザーがViewerでダブルクリックした部品のAssemblyパス・名前・バウンディングボックスを取得する（言葉で場所を説明する代わりに使う）
- `get_viewer_state`: 現在のカメラ位置・注視点・アニメーション再生位置・非表示パーツを取得する（今どこを見ているか、どの時点で止めているかを言葉で説明する代わりに使う）
